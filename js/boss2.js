...
$(document).ready(function() {
  ...
  if (!(($("#lifePerson").width() / $("tfoot").width() * 100 > 0 && $("#lifeEnemy").width() / $("#enemyBaal").width() * 100 <= 0) ||
      ($("#lifePerson").width() / $("tfoot").width() * 100 <= 0))) {
...
function proximoAtaque() {
  ...
  if ($("#lifePerson").width() / $("tfoot").width() * 100 <= 0) {
    var total = parseInt($("#acerto").val());
    total += parseInt($("#acertoAcm").val());
	$("#endFigth").empty();
    $("#endFigth").html("Voce foi Derrotado e sua alma foi consumida pela Hidra de Lerna<br />" +
                        "Total de acertos: " + $("#acerto").val() + ", Total de Erros: " + $("#erros").val() + "<br />" +
